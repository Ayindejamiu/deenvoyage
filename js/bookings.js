...
function updateSummary() {
  ...
  summary.price.textContent = `$${selectedPackage.price.toLocaleString()} / person`;
}

function setupPackageSelection() {
  pkgCards.forEach((card) => {
    card.addEventListener("click", () => {
      pkgCards.forEach((item) => item.classList.remove("selected"));
...
function setupOccupancySelection() {
  occButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      occButtons.forEach((item) => item.classList.remove("selected"));
...
function openWhatsAppChat(payload) {
  ...
  const message = [
    ...
    `*Starting Price:* $${payload.packageStartPrice.toLocaleString()} / person`,
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    showStatus("Booking request saved successfully. Our team will contact you shortly.");
    ...
    pkgCards.forEach((item, index) => item.classList.toggle("selected", index === 0));
    occButtons.forEach((item, index) => item.classList.toggle("selected", index === 0));
    updateSummary();
  } catch (error) {
